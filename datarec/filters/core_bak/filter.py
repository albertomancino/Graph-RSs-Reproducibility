...
class Filter:
    ...
    def filter(self, data: Union[pd.DataFrame, datarec.DataRec]):
        t_ = type(data)
        if t_ not in {pd.DataFrame, datarec.DataRec}:
            raise ValueError(f'data type {t_} is not supported')
        if t_ == pd.DataFrame:
            self.filter_engine(data)
        else:
            self.filter_engine(data.data)
