...
from datarec.io.paths import RESULT_DIR
...
class DataRecWriter(Writer):
    def write(self, data: DataRec, path, verbose=False) -> None:
        """
        Write data following the DataRec Format. Store data at path.
        :param data: a DataRec object containing the data to be written
        :param path: output file path
        :param verbose: print debugging information
        """
        data.data.to_csv(path, sep='\t', header=False, index=False)
        if verbose:
            print(f'Dataset stored at \'{path}\'')


class InlineItemsWriter(Writer):
    pass
